...
License: GPL
Requires: module-init-tools
Summary: Lustre networking for CLE VMs

%description
Kernel modules and userspace tools needed for a Lustre client on CLE VMs.

%description lnet
Userspace tools and files for Lustre networking on CLE VMs

%package -n cray-lustre-%{flavor}-%{vendor_version}-devel
Group: Development/Libraries
License: GPL
Summary: Cray Lustre Header files
Provides: cray-lnet-%{vendor_version}-devel

%description -n cray-lustre-%{flavor}-%{vendor_version}-devel
...
%install
# don't use %makeinstall for these RPMS - it needlessly puts things into 
# /opt/cray/...
...
man_path="opt/cray/lustre-%{flavor}/%{version}-%{release}/man"
if [ -e ${man_path} ]; then
...
%files -n cray-lustre-%{flavor}-%{vendor_version}-devel
%defattr(-,root,root)
