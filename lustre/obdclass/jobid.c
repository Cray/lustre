...
static int jobid_should_free_item(void *obj, void *data)
{
	...
	if (jobid == NULL)
		...
		rc = 1;
	else if (cfs_time_current_sec() - pidmap->jp_time > DELETE_INTERVAL)
		rc = 1;
...
int get_jobid(char *jobid)
{
	...
	spin_lock(&pidmap->jp_lock);
	if ((cfs_time_current_sec() - pidmap->jp_time >= RESCAN_INTERVAL) ||
	    pidmap->jp_jobid[0] == '\0') {
		...
		cfs_time_t tmp_time = pidmap->jp_time;
		pidmap->jp_time = cfs_time_current_sec();
...
int lustre_get_jobid(char *jobid)
{
	int rc = 0;
	int clear = 0;
	static cfs_time_t last_delete;
	ENTRY;

	LASSERT(jobid_hash != NULL);

	spin_lock(&jobid_hash_lock);
	if (last_delete + DELETE_INTERVAL <= cfs_time_current_sec()) {
		clear = 1;
		last_delete = cfs_time_current_sec();
	}
	spin_unlock(&jobid_hash_lock);
