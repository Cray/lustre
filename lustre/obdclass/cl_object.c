...
static struct lu_env *cl_env_obtain(void *debug)
{
	...
	ENTRY;
	read_lock(&cl_envs[cpu].guard);
	LASSERT(equi(cl_envs[cpu].nr == 0, cfs_list_empty(&cl_envs[cpu].envs)));
	if (cl_envs[cpu].nr > 0) {
		int rc;

		cle = container_of(cl_envs[cpu].envs.next, struct cl_env, ce_linkage);
		cfs_list_del_init(&cle->ce_linkage);
		cl_envs[cpu].nr--;
		read_unlock(&cl_envs[cpu].guard);
		put_cpu();
...
unsigned cl_env_cache_purge(unsigned nr)
{
	...
	for_each_possible_cpu(i) {
		write_lock(&cl_envs[i].guard);
		for (; !cfs_list_empty(&cl_envs[i].envs) && nr > 0; --nr) {
			cle = container_of(cl_envs[i].envs.next,
					   struct cl_env, ce_linkage);
			cfs_list_del_init(&cle->ce_linkage);
			LASSERT(cl_envs[i].nr > 0);
			cl_envs[i].nr--;
			write_unlock(&cl_envs[i].guard);

			cl_env_fini(cle);
			write_lock(&cl_envs[i].guard);
		}
		LASSERT(equi(cl_envs[i].nr == 0,
			     cfs_list_empty(&cl_envs[i].envs)));
		write_unlock(&cl_envs[i].guard);
	}
	RETURN(nr);
}
...
void cl_env_put(struct lu_env *env, int *refcheck)
{
		...
		int cpu = get_cpu();
                 ...
                 */
                if (cl_envs[cpu].nr < cl_envs_cached_max &&
                    (env->le_ctx.lc_tags & ~LCT_HAS_EXIT) == LCT_CL_THREAD &&
                    (env->le_ses->lc_tags & ~LCT_HAS_EXIT) == LCT_SESSION) {
			read_lock(&cl_envs[cpu].guard);
			cfs_list_add(&cle->ce_linkage, &cl_envs[cpu].envs);
			cl_envs[cpu].nr++;
			read_unlock(&cl_envs[cpu].guard);
		} else
			cl_env_fini(cle);
...
static int cl_env_percpu_init(void)
{
	...
	for_each_possible_cpu(i) {
		...
		rwlock_init(&cl_envs[i].guard);
		CFS_INIT_LIST_HEAD(&cl_envs[i].envs);
		cl_envs[i].nr = 0;
