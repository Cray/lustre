...
static struct lu_env *cl_env_obtain(void *debug)
{
	...
	LASSERT(equi(cl_envs[cpu].nr == 0, list_empty(&cl_envs[cpu].envs)));
	if (cl_envs[cpu].nr > 0) {
		int rc;

		cle = container_of(cl_envs[cpu].envs.next, struct cl_env,
				   ce_linkage);
...
unsigned cl_env_cache_purge(unsigned nr)
{
	...
	for_each_possible_cpu(i) {
		...
		for (; !list_empty(&cl_envs[i].envs) && nr > 0; --nr) {
...
void cl_env_put(struct lu_env *env, int *refcheck)
{
		...
		int cpu = get_cpu();
			...
			read_lock(&cl_envs[cpu].guard);
			list_add(&cle->ce_linkage, &cl_envs[cpu].envs);
			cl_envs[cpu].nr++;
			read_unlock(&cl_envs[cpu].guard);
		} else
			cl_env_fini(cle);
