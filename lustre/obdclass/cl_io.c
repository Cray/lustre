                            ...
                            const struct cl_lock_descr *need)
{
	...
	ENTRY;
	cfs_list_for_each_entry(scan, &set->cls_todo, cill_linkage) {
                           ...
                           struct cl_lockset *set)
{
	...
	cfs_list_for_each_entry_safe(link, temp, &set->cls_todo, cill_linkage) {
		...
		cfs_list_move(&link->cill_linkage, &set->cls_done);
        }
	RETURN(result);
}
 ...
 *
 * \see cl_io_operations::cio_readahead()
 */
		     ...
		     pgoff_t start, struct cl_read_ahead *ra)
{
        ...
        cl_io_for_each(scan, io) {
                if (scan->cis_iop->cio_read_ahead != NULL) {
                        result = scan->cis_iop->cio_read_ahead(env, scan,
							       start, ra);
                        if (result != 0)
                                break;
                }
		     ...
		     void (*end)(const struct lu_env *, struct cl_sync_io *))
{
	...
	cfs_atomic_set(&anchor->csi_sync_nr, nr);
	cfs_atomic_set(&anchor->csi_barrier, nr > 0);
		    ...
		    long timeout)
{
	...
	if (rc < 0) {
		...
		if (anchor->csi_cancel != NULL)
			rc2 = anchor->csi_cancel(env, anchor);

		CERROR("SYNC IO failed with error: %d, try to cancel "
		       "%d remaining entities: %d\n",
		       rc, cfs_atomic_read(&anchor->csi_sync_nr), rc2);
