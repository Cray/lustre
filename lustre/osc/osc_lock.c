...
int osc_ldlm_glimpse_ast(struct ldlm_lock *dlmlock, void *data)
{
	...
	struct ldlm_match_data  matchdata;
	union ldlm_policy_data  policy;
	...
	matchdata.lmd_old = NULL;
	matchdata.lmd_lock = NULL;
	...
	while (dlmlock) {
		...
		if (obj == NULL && res->lr_type == LDLM_EXTENT) {
			if (OBD_FAIL_CHECK(OBD_FAIL_OSC_NO_SIZE_DATA))
				break;
			/* search_itree returns the contents of lmd_lock, so we
			 * must clear it first or we may get a stale result
			 */
			matchdata.lmd_lock = NULL;
			lock_res(res);
					   ...
					   enum osc_dap_flags dap_flags)
{
	...
	enum ldlm_mode mode;
	__u64 flags;
	enum ldlm_match_flags m_flags = 0;
	...
	if (dap_flags & OSC_DAP_FL_AST)
		m_flags |= LDLM_MATCH_AST;

	if (dap_flags & OSC_DAP_FL_CANCELING)
		m_flags |= LDLM_MATCH_UNREF;

	if (dap_flags & OSC_DAP_FL_RIGHT)
		m_flags |= LDLM_MATCH_RIGHT;
