			  ...
			  enum cl_lock_mode mode, int discard)
{
	...
	rc = osc_lock_discard_pages(env, obj, start, end, mode);
	if (result == 0 && rc < 0)
		result = rc;
...
unsigned long osc_ldlm_weigh_ast(struct ldlm_lock *dlmlock)
{
	...
	cfs_list_for_each_entry(oscl, &obj->oo_ol_list, ols_nextlock_oscobj) {
	...
	if (found) {
		/*
		 * If the lock is being used by an IO, definitely not cancel
		 * it
		 */
		GOTO(out, weight = 1);
	}
				  ...
				  struct osc_lock *oscl)
{
	...
	cfs_list_del_init(&oscl->ols_nextlock_oscobj);
	while (!cfs_list_empty(&oscl->ols_waiting_list)) {
		struct osc_lock	*scan;

		scan = cfs_list_entry(oscl->ols_waiting_list.next,
				      struct osc_lock, ols_wait_entry);
		cfs_list_del_init(&scan->ols_wait_entry);
				  ...
				  struct osc_lock *oscl)
{
	...
	spin_lock(&obj->oo_ol_spin);
	cfs_list_add_tail(&oscl->ols_nextlock_oscobj, &obj->oo_ol_list);

restart:
	cfs_list_for_each_entry(tmp_oscl, &obj->oo_ol_list,
				ols_nextlock_oscobj) {
		...
		cfs_list_add_tail(&oscl->ols_wait_entry,
		  ...
		  const struct cl_io *io)
{
	...
	if (oscl != NULL) {
		...
		CFS_INIT_LIST_HEAD(&oscl->ols_waiting_list);
		CFS_INIT_LIST_HEAD(&oscl->ols_wait_entry);
		CFS_INIT_LIST_HEAD(&oscl->ols_nextlock_oscobj);
