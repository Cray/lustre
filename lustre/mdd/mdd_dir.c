			    ...
			    int idx, struct thandle *th)
{
	...
	CDEBUG(D_TRACE, "Adding rec %u type %u size %d to "DFID" off %lld flags %x count %d\n",
	       idx, r->lrh_type, r->lrh_len, PLOGID(&loghandle->lgh_id),
	       loghandle->lgh_cur_offset, loghandle->lgh_hdr->llh_flags,
	...
	if (r->lrh_type == CHANGELOG_REC) {
		...
		if (unlikely(th != saved_th)) {
			CDEBUG(D_OTHER, "%s: wrote rec %u off %llu "DFID" count %d\n",
			       loghandle->lgh_ctxt->loc_obd->obd_name,
			       r->lrh_index, loghandle->lgh_cur_offset,
