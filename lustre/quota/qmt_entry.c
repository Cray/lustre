			  ...
			  const char *fmt, va_list args)
{
	...
	libcfs_debug_vmsg2(msgdata, fmt, args,
			   "qmt:%s pool:%s-%s id:%llu enforced:%d hard:%llu soft:%llu granted:%llu time:%llu qunit: %llu edquot:%d may_rel:%llu revoke:%lld default:%s\n",
...
inline __u64 qti_lqes_min_qunit(const struct lu_env *env)
{
	__u64 min, qunit;
	int i;
