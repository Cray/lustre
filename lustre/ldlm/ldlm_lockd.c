...
time64_t ldlm_bl_timeout_by_rpc(struct ptlrpc_request *req)
{
	...
	time64_t timeout, req_timeout, at_timeout, netl;
	...
	return max(timeout + (timeout >> 4), (time64_t)ldlm_enqueue_min);
}
                             ...
                             struct ptlrpc_request *req, void *data, int rc)
{
	...
	case LDLM_GL_CALLBACK:
		...
		if (unlikely(arg->gl_interpret_reply)) {
			rc = arg->gl_interpret_reply(NULL, req, data, rc);
			       ...
			       struct ldlm_bl_work_item *blwi)
{
	...
	if (blwi->blwi_count) {
		...
		ldlm_handle_bl_callback(blwi->blwi_ns, &blwi->blwi_ld,
                ...
                ldlm_pool_recalc(&blwi->blwi_ns->ns_pool, 1);
