...
{
	struct semaphore_timer *timeout = from_timer(timeout, t, timer);
			     ...
			     int comp_idx, __u64 reserve)
{
	...
	if (!down_write_trylock(&lod->lod_ost_descs.ltd_qos.lq_rw_sem)) {
		...
		timer.task = current;
		timer_setup_on_stack(&timer.timer, process_semaphore_timer, 0);
		...
		if (rc) {
			...
			CDEBUG(D_QOS, "wakeup semaphore on timeout rc = %d\n",
			       rc);
			GOTO(out_nolock, rc = -EAGAIN);
		}
	}
