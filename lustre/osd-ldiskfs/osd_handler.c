			       ...
			       int *err)
{
	...
	if (IS_ERR(inode)) {
		...
		/**
		 * EA is internal ldiskfs object, shouldn't be visible on osd
		 */
				  ...
				  loff_t offset, __u64 ino, unsigned int d_type)
{
	...
	if (OBD_FAIL_CHECK(OBD_FAIL_OFD_IGET_FAIL)) {
		oclb->oclb_rc = -ESTALE;
		RETURN(-ESTALE);
	}
...
again:
	...
	OBD_FAIL_CHECK_RESET(OBD_FAIL_OFD_IGET_FAIL_TO_START, OBD_FAIL_OFD_IGET_FAIL);
	do {
		oclb.oclb_items = 0;
		oclb.oclb_rc = 0;
	...
	OBD_FAIL_CHECK_RESET(OBD_FAIL_OFD_IGET_FAIL, 0);
...
int osd_ldiskfs_it_fill(const struct lu_env *env, const struct dt_it *di)
{
	...
	rc = down_read_killable(&ino->i_rwsem);
	if (rc)
		GOTO(unlock, rc);
