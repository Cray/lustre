...
#define MIGRATE_USAGE							\
	...
	"\tarchive:      HSM archive index number to use for the data " \
	"                migration,\n"					\
	"\t              only valid with --hsm\n"
...
command_t cmdlist[] = {
	...
	{"setquota", lfs_setquota, 0, "Set filesystem quotas.\n"
	 ...
	 "		  <-d|--default> [--pool <OST pool name>] <filesystem>\n"
		   ...
		   char *volatile_file)
{
	...
	if (migration_flags & MIGRATION_HSM)
		volatile_name = ".HSM";
	else
	...
	do {
		...
		rc = snprintf(volatile_file, PATH_MAX,
			      "%s/%s:%.4X:%.4X", parent, volatile_name,
			   ...
			   int archive_id)
{
	...
	hur->hur_request.hr_flags = (migration_flags & MIGRATION_NONBLOCK) ?
...
static int mirror_extend_layout(char *name, struct llapi_layout *layout)
{
	...
	rc = migrate_open_files(name, 0, NULL, layout, &fd, &fdv, NULL);
	if (rc < 0)
		goto out;
...
/* find	{ .val = 'U',	.name = "user",		.has_arg = required_argument }*/
	...
	if (archive_id != 0 && !(migration_flags & MIGRATION_HSM)) {
		fprintf(stderr,
			   ...
			   bool h, __u64 *total)
{
	...
	/* Is it correct for the case OST000, OST0002, OST003 -
...
static int lfs_changelog(int argc, char **argv)
{
	...
	while ((rc = llapi_changelog_recv(changelog_priv, &rec)) == 0) {
		...
		if (rec->cr_namelen)
			printf(" %.*s", rec->cr_namelen, changelog_rec_name(rec));
