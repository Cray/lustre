...
static inline unsigned int node_limit(struct iam_params *params)
{
	unsigned int entry_space   = params->blocksize -
		params->node_gap;
	return entry_space / (params->keysize + params->ptrsize);
}


static inline unsigned int root_limit(struct iam_params *params)
{
	unsigned int limit = params->blocksize -
		params->root_gap;
	limit /= (params->keysize + params->ptrsize);
	if (limit == node_limit(params))
		limit--;
	return limit;
}
...
static int check_index(char *buf, struct iam_params *params)
{
	...
	counted_limit = node_limit(params);
...
static int check_root(char *buf, size_t size, struct iam_params *params)
{
	...
	if (root->ilr_magic == htole64(IAM_LFIX_ROOT_MAGIC)) {
		params->fmt = FMT_LFIX;
		if (verbose)
			printf("LFIX,");
	...
	counted_limit = root_limit(params);
