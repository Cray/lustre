...
# first OBD_FAIL_MGC_FAIL_NET            0x910 used to hit
#       OBD_FAIL_MGC_FS_CLEANUP_RACE     0x911
	$LCTL set_param fail_loc=0x910
	start_ost
	cleanup
}
...
test_115() {
	...
	trap "cleanup_115 $oldimagesize" ERR EXIT

	tmpmdsdevname=$(mdsdevname 1)
	local mds_opts="$(mkfs_opts mds1 $tmpmdsdevname) --device-size=$IMAGESIZE   \
...
#define OBD_FAIL_OFD_SET_OID 0x1e0
	...
	err=$(do_facet ost1 dmesg | tac | sed "/Recovery over/,$ d" |
	      grep "Too many FIDs to precreate OST replaced")
	[ -z "$err" ] || error $err
}
