...
ha_start_mpi_loads()
{
	...
	(( inst == 0 )) && ha_info "no mpi load to start, ha_mpi_instances=$inst" &&
		return 0
	(( ${#ha_mpi_load_tags[@]} == 0 )) && ha_info "no mpi load to start" &&
		return 0
	...
	for ((n = 0; n < $inst; n++)); do
		...
		for ((load = 0; load < ${#ha_mpi_load_tags[@]}; load++)); do
			...
			ha_repeat_mpi_load $client $load $status "$parameter" $machines \
				"$stripe" "$mpiuser" "$mustpass" \
...
ha_power_down()
{
	...
	[ $rc -eq 0 ] || {
		ha_info "Failed Powering down in $i attempts: $ha_power_down_cmd"
		cat $ha_pm_states
		exit 1
	}
}
...
ha_power_up_delay()
{
	...
	while (($(date +%s) <= end)); do
		...
		for n in ${nodes//,/ }; do
			...
			local status=$(ha_on $pair crm_mon -1rQ | grep -w $n | head -1)
