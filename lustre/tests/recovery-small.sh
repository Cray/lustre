...
#define OBD_FAIL_OST_PREPARE_DELAY	 0x243
	do_facet ost1 "$LCTL set_param fail_loc=0x243"
        # disable pinger recovery
        $LCTL set_param -n osc.$OST1_IMP.pinger_recov=0
...
test_134() {
	...
	do_facet mds1 "$LCTL set_param fail_loc=0x721 fail_val=5"
...
run_test 136 "late resend must be skipped if already applied"
