...
test_26b() {      # bug 10140 - evict dead exports by pinger
	...
	local INTERVAL=$(do_facet mds1 lctl get_param -n ping_interval)
	local AT_MAX_SAVED=$(at_max_get mds1)
	local ldlm_enqueue_min=$(do_facet mds1 "find /sys/module \
				 -name ldlm_enqueue_min")
	local ENQUEUE_MIN=$(do_facet mds1 cat $ldlm_enqueue_min)
...
test_144c() {
	...
	stop mds1
#define OBD_FAIL_OST_DELORPHAN_DELAY     0x253
	#delay delorphan request to reconnection 5seconds
	do_facet ost1 $LCTL set_param fail_loc=0x0000253 fail_val=5
...
test_152() {
	...
	do_facet mds1 $LCTL set_param fail_loc=0x8000016f fail_val=20
	before=$(date +%s)
	for (( i = 0; i < 2; i++)); do
	...
	(( $diff < 20 )) || error "QoS allocation slower than RR, killable semaphore doesn't work"
}
...
test_158a() {
	(( $MDS1_VERSION >= $(version_code 2.15.8) )) ||
		skip "Need MDS version at least 2.15.8 for -EACCES fix"
...
test_160() {
	(( $MDS1_VERSION >= $(version_code 2.15.4) )) ||
		skip "Need MDS version at least 2.15.4 for group lock destroy fix"
