...
test_44b() {
	(( $OST1_VERSION >= $(version_code 2.15.4.1) )) ||
		skip "Need OST >= 2.15.4.1 for large object handling"
...
run_test 44g "write big sparse write"
...
test_105g() {
	...
	(( $CLIENT_VERSION >= $(version_code 2.15.4) )) ||
		skip "Need Client >= 2.15.4 for ldlm dump stack"
...
run_test 124f "clearing lustre caches with lru_size"
...
run_test 124g "clearing lustre caches in parallel with drop_caches"
...
test_130h() {
	...
	# check for client eviction
        $LFS df -h $DIR
        $LCTL get_param osc.$FSNAME-OST0001-osc-f*.state
...
test_230A()
{
        ...
        (( MDS1_VERSION >= $(version_code 2.15.6) )) ||
                skip "need MDS >= 2.15.6 for lmm_oi migrate update"
...
test_270j() {
	(( $MDS1_VERSION >= $(version_code 2.15.1.2) )) ||
		skip "Need MDS version at least 2.15.1.2"
...
run_test 413g "check default layout inheritance"
