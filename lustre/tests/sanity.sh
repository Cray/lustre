...
if [[ $(uname -m) = aarch64 ]]; then
	...
	# bug number:	 LU-11671 LU-11665 LU-11594 LU-11667 LU-11729 LU-4398
	ALWAYS_EXCEPT+=" 45	  101c	   103a	    317      810      817"
fi
...
test_64e() {
	...
	# Remount client to reset grant
	remount_client $MOUNT || error "failed to remount client"
	instance=$($LFS getname $DIR | sed 's/.*-\(.*\) .*/\1/')
	osc_tgt="$FSNAME-OST0000-osc-$instance"
...
test_64g() {
	...
	local num_exps=$(do_facet ost1 \
	    $LCTL get_param -n obdfilter.*OST0000*.num_exports | cut -d' ' -f2)
	local max_brw_size=$(import_param $osc_tgt max_brw_size)
	...
	tot_granted[0]=$(do_facet ost1 \
	    $LCTL get_param -n obdfilter.*OST0000*.tot_granted | cut -d' ' -f2)
...
run_test 64g "grant shrink on read"
...
run_test 124e "clearing lustre caches with lru_size"
...
run_test 124f "clearing lustre caches in parallel with drop_caches"
