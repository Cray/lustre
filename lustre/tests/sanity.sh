...
# bug number for skipped test:        LU-1593 LU-2610 LU-2833 LU-5242  LU-1957 LU-2805
	ALWAYS_EXCEPT="$ALWAYS_EXCEPT 34h     40      48a     78 79 80 180     184c"
...
test_27D() { # LU-5346/LU-7975

	[ $PARALLEL == "yes" ] && skip "skip parallel run" && return

	[[ $(lustre_version_code $SINGLEMDS) -lt $(version_code 2.5.42) ]] &&
		skip "Need MDS version at least 2.5.42" && return
...
run_test 27D "Client resend delayed layout creation with non-zero size"
...
test_101f() {
	...
	# create a test file, ignore errors
	...
	[ $miss -lt 10 ] || error "misses too much pages!"
}
...
test_103a() {
	...
	# INTL-144 mdd: Setting xattr are properly checked with and without ACLs
