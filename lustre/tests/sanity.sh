...
ALWAYS_EXCEPT="  253     312     160f	160g	$ALWAYS_EXCEPT"
...
test_56ba() {
	[ $(lustre_version_code $SINGLEMDS) -lt $(version_code 2.10.50) ] &&
		skip "Need MDS version at least 2.10.50" && return
...
test_224b() { # LU-1039, MRP-303
	...
	do_facet ost1 $LCTL set_param fail_val=3 fail_loc=0x80000515
	dd of=$TMP/$tfile.new if=$DIR/$tfile bs=1M count=1 || rc1=$?
	do_facet ost1 $LCTL set_param fail_loc=0
...
test_224d() { # LUS-6595
	...
	#define OBD_FAIL_PTLRPC_CLIENT_BULK_CB2   0x515
	do_facet ost1 $LCTL set_param fail_val=$(($TIMEOUT + 2)) fail_loc=0x80000515
	dd of=$TMP/$tfile.new if=$DIR/$tfile bs=1M count=1 || rc1=$?
	do_facet ost1 $LCTL set_param fail_loc=0
	# restore adaptive timeout
	[ $at_max_saved -ne 0 ] && at_max_set $at_max_saved mds client
...
run_test 425 "rename striped directory"
