...
test_244() {
	...
	# This test case is time sensitive and auto-test uses kvm to run auto
	# test. Therefore the complete time of I/O task is unreliable and
	# depends on the work load on the host machine when the task is running.
	which virt-what 2> /dev/null && [ "$(virt-what)" != "kvm" ] ||
		{ echo "no virt-what installed or running in kvm; ignore error";
	...
	# verify that fast read is 4 times faster for cache read
	[ $(bc <<< "4 * $s1 < $s2") -eq 1 ] ||
		$my_error "fast read was not 4 times faster: $s1 vs $s2"
	...
	# 1M non-cache read
	cancel_lru_locks osc
	s2=$(eval time -p dd if=$DIR/$tfile of=/dev/null bs=1M 2>&1 |
	     awk '/real/ { print $2 }')

	# verify that big IO is not 4 times faster than small IO
	[ $(bc <<< "4 * $s2 >= $s1") -eq 1 ] ||
		$my_error "bigger IO is way too fast: $s1 vs $s2"

	$LCTL set_param -n llite.*.fast_read=$saved_fast_read
	rm -f $DIR/$tfile
}
run_test 244 "fast read verification"
