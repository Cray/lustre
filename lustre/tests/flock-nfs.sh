...
nfs_start() {
	...
	# setup the nfs
	if ! setup_nfs "$NFSVERSION" "$NFS_MNT" \
			"$lustre_client" "$NFS_CLIENT"; then
...
#define OBD_FAIL_LLITE_FLOCK_UNLOCK_RACE       0x1450
	do_node $lustre_client "lctl set_param fail_loc=0x1450"
	do_node $lustre_client \
		"flocks_test 5 set write sleep 10 $LUSTRE_MNT/$LOCKF" &
	sleep 1
...
#define OBD_FAIL_LLITE_FLOCK_BL_GRANT_RACE     0x1451
	do_node $lustre_client "lctl set_param fail_loc=0x1451"
	do_node $NFS_CLIENT "flocks_test 5 set write sleep 5 $LUSTRE_MNT/$LOCKF"
	wait
