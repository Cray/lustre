...
AC_DEFUN([LC_SRC_HAVE_RB_ADD], [
	LB2_LINUX_TEST_SRC([rb_add], [
		#include <linux/rbtree.h>
	],[
		rb_add(NULL, NULL, NULL);
	])
])
AC_DEFUN([LC_HAVE_RB_ADD], [
	LB2_MSG_LINUX_TEST_RESULT([if 'rb_add' exists in rbtree.h],
	[rb_add], [
		AC_DEFINE(HAVE_RB_ADD, 1,
			[kernel has rb_add in rbtree.h])
	])
]) # LC_HAVE_RB_ADD
