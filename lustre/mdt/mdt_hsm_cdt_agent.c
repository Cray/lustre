		       ...
		       bool purge)
{
	...
	CDEBUG(D_HSM, "Agent %s selected for archive %d request %px items %d\n",
	       obd_uuid2str(&uuid), archive_id, rq, rq->hal_count);
...
update_records:
	...
	list_for_each_entry(car, &rq->hal_cars, car_scan_list) {
		...
		rc2 = mdt_hsm_agent_modify_record(mti->mti_env, mdt,
						  car->car_hmm);
		if (rc2)
			CERROR("%s: mdt_agent_record_update() failed, cannot update status to %s for cookie %#llx: rc = %d\n",
			       mdt_obd_name(mdt),
