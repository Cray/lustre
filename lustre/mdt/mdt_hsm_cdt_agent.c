				    ...
				    struct hsm_action_item *hai)
{
	...
	list_for_each_entry(ha, &cdt->cdt_agents, ha_list) {
		for (i = 0; i < ha->ha_archive_cnt; i++) {
			...
			CDEBUG(D_HSM, "%s: added HSM remove request for "DFID
			       ", archive_id=%d\n", mdt_obd_name(mti->mti_mdt),
			       PFID(&hai->hai_fid), ha->ha_archive_id[i]);
		}
		       ...
		       struct hsm_action_list *hal, bool purge)
{
	...
	if (rc < 0 && hal->hal_archive_id == 0) {
		unsigned int non_remove_count = 0;
		int rc2 = 0;
		...
		/* only remove requests with archive_id=0 have been processed */
		if (non_remove_count == 0)
			RETURN(rc2);
	}
