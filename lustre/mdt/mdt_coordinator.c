			      ...
			      void *data)
{
	...
	case ARS_STARTED: {
		...
		hdr->lrh_len = REC_DATA_LEN(hdr);
		rc = llog_write(hsd->mti->mti_env, llh, hdr, NULL, 0,
				REC_DATA(hdr), hdr->lrh_index);
		if (rc < 0)
...
int mdt_hsm_cdt_init(struct mdt_device *mdt)
{
	...
	CFS_INIT_LIST_HEAD(&cdt->cdt_request_list);
			     ...
			     struct llog_rec_hdr *hdr, void *data)
{
	...
	if (larr->arr_status == ARS_WAITING ||
		...
		hdr->lrh_len = REC_DATA_LEN(hdr);
		rc = llog_write(env, llh, hdr, NULL, 0, REC_DATA(hdr),
				hdr->lrh_index);
	}
