...
static inline int mdt_cdt_check_rec(struct llog_agent_req_rec *rec, size_t size)
{
	...
	if (rec->arr_hdr.lrh_len > size ||
	    ...
	    rec->arr_hdr.lrh_index != llog_rec_tail(&rec->arr_hdr)->lrt_index ||
	    llog_rec_tail(&rec->arr_hdr)->lrt_len != rec->arr_hdr.lrh_len)
		RETURN(-EBADR);
	...
	if (rec->arr_hai.hai_len > rec->arr_hdr.lrh_len ||
	    rec->arr_hai.hai_len < sizeof(struct hsm_action_item) ||
			      ...
			      struct hsm_scan_data *hsd)
{
	...
	if (hsd->hsd_action_count + atomic_read(&cdt->cdt_request_count) >=
		...
		if (request) {
			...
			do {
				...
				car = list_last_entry(&request->hal_cars,
			...
			hsd->hsd_action_count -= tmp->hal_count;
			LASSERT(hsd->hsd_action_count >= 0);
			list_for_each_entry_safe(pos, tmp2, &tmp->hal_cars,
						 car_scan_list){
	...
	if (!request) {
		...
		INIT_LIST_HEAD(&request->hal_cars);
		request->hal_fsname = &hsd->hsd_fsname[0];
		request->hal_version = HAL_VERSION;
		request->hal_count = 0;
		request->hal_used_sz = sizeof(struct hsm_action_list) +
	...
	car->car_hmm->mr_offset = llog_get_record_off(env,
					      (struct llog_rec_hdr *)larr);
	LASSERT(car->car_hmm->mr_offset >= llh->lgh_hdr->llh_size);
	car->car_hmm->mr_lid = llh->lgh_id;
	list_add_tail(&car->car_scan_list, &request->hal_cars);

	CDEBUG(D_HSM, "Copying record %d to request %px, count %d\n",
	       larr->arr_hdr.lrh_index, request, request->hal_count);
			      ...
			      void *data)
{
	...
	if (rc) {
		CDEBUG(D_HSM, "%s: bad llog record at "DOSTID" idx %d, rc = %d\n",
...
clean_cb_alloc:
		...
		for (i = 0; i < hsd.hsd_request_count; i++) {
			struct hsm_scan_request *request = &hsd.hsd_request[i];
			struct cdt_agent_req *pos, *tmp;

			list_for_each_entry_safe(pos, tmp, &request->hal_cars,
						 car_scan_list){
				list_del_init(&pos->car_scan_list);
				mdt_cdt_put_request(pos);
			}
...
static int mdt_hsm_cdt_start(struct mdt_device *mdt)
{
	...
	atomic64_set(&cdt->cdt_last_cookie, 0);
		    ...
		    struct obd_uuid *uuid)
{
	...
	list_for_each_entry(car, &rq->hal_cars, car_scan_list) {
		hmm = car->car_hmm;
		...
		if (hai->hai_action == HSMA_CANCEL) {
			...
			h = &orig->car_hai;
			if (orig->car_canceled) {
				CDEBUG(D_HSM, "%s: record %llx already canceled %s "DFID"\n",
			...
			if (rc) {
				CERROR("%s: mdt_agent_modify_record() failed, cannot update status to %s for cookie %#llx : rc = %d\n",
				       mdt_obd_name(mdt),
			    ...
			    struct md_hsm *mh_common)
{
	...
	if (!rc)
		...
		CDEBUG(D_INODE, "%s: cannot get parent FID for "DFID"\n",
		       mdt_obd_name(mti->mti_mdt), PFID(mdt_object_fid(obj)));
				 ...
				 struct hsm_progress_kernel *pgs)
{
	...
	if (pgs->hpk_flags & HP_FLAG_COMPLETED) {
		...
		if (hmm->mr_rec.arr_status == ARS_STARTED ||
			...
			if (rc1)
				CERROR("%s: mdt_agent_record_update() failed, rc=%d, cannot update status to %s for cookie %#llx\n",
...
static int hsm_cancel_all_actions(struct mdt_device *mdt)
{
	...
	struct hsm_scan_request rq = {.hal_version = HAL_VERSION,
				      .hal_count = 1,
				      .hal_fsname = &fsname[0],};
	...
	hsm_init_ucred(mdt_ucred(mti));
	obd_uuid2fsname(rq.hal_fsname, mdt_obd_name(mdt), MTI_NAME_MAXLEN);
	...
	list_for_each_entry(car, &cdt->cdt_request_list, car_request_list) {
		...
		INIT_LIST_HEAD(&rq.hal_cars);
		list_add_tail(&car->car_scan_list, &rq.hal_cars);
