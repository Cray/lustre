...
struct cdt_agent_req {
	...
	bool			 car_canceled;     /**< request was canceled */
...
static inline u32 hsr_get_archive_id(struct hsm_scan_request *rq)
{
	...
	if (rq->hal_count > 0) {
		car = list_first_entry(&rq->hal_cars, struct cdt_agent_req,
				       car_scan_list);
...
static inline size_t hsr_hal_size(struct hsm_scan_request *rq)
{
	struct cdt_agent_req *car;
	...
	sz = sizeof(struct hsm_action_list) +
	     __ALIGN_KERNEL(strlen(rq->hal_fsname) + 1, 8);
...
static inline int hsr_hal_copy(struct hsm_scan_request *rq, void *buf,
			       size_t buf_size)
{
	struct hsm_action_list *hal = buf;
	struct cdt_agent_req *car;
	struct hsm_action_item *hai, *shai;

	hal->hal_version = rq->hal_version;
	strscpy(hal->hal_fsname, rq->hal_fsname, MTI_NAME_MAXLEN + 1);
...
int mdt_hsm_add_hsr(struct mdt_thread_info *mti, struct hsm_scan_request *hal,
		    struct obd_uuid *uuid);
