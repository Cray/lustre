                       ...
                       const char *format2, ...)
{
	...
	int (*_func)(char *, size_t,  const char *, va_list);
	 ...
	 */
	for (retry = 0, _func = vsnprintf; retry < 2; retry++) {
		...
		if (format1) {
			va_copy(ap, args);
			needed = _func(string_buf, max_nob, format1, ap);
			va_end(ap);
		}

		if (format2) {
			char *soff = string_buf + needed;
			int avail = max_t(int, max_nob - needed, 0);

			va_start(ap, format2);
			needed += _func(soff, avail, format2, ap);
			va_end(ap);
		}

		if (needed < max_nob) /* well. printing ok.. */
			break;
		_func = vscnprintf; /* second pass: used vscnprintf */
	}
...
console:
	...
	if (tcd) {
		...
		if (format1 != NULL) {
			va_copy(ap, args);
			needed = vscnprintf(string_buf,
					    CFS_TRACE_CONSOLE_BUFFER_SIZE,
					    format1, ap);
			va_end(ap);
		}
		if (format2 != NULL) {
			int remain = CFS_TRACE_CONSOLE_BUFFER_SIZE - needed;
			if (remain > 0) {
				va_start(ap, format2);
				needed += vscnprintf(string_buf+needed, remain,
						     format2, ap);
				va_end(ap);
			}
		}
