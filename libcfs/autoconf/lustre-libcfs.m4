...
#
AC_DEFUN([LIBCFS_SRC_HAVE_SOCKET_SK_SK_REUSEPORT], [
	LB2_LINUX_TEST_SRC([socket_sk_sk_reuseport], [
		#include <net/sock.h>
	],[
...
AC_DEFUN([LIBCFS_HAVE_SOCKET_SK_SK_REUSEPORT], [
	LB2_MSG_LINUX_TEST_RESULT([if 'struct socket' has 'sk->sk_reuseport' member],
...
# v3.9
#
AC_DEFUN([LIBCFS_SRC_HAVE_IDR_ALLOC], [
	LB2_LINUX_TEST_SRC([idr_alloc_fn], [
		#include <linux/idr.h>
	],[
...
AC_DEFUN([LIBCFS_HAVE_IDR_ALLOC], [
	LB2_MSG_LINUX_TEST_RESULT([if 'idr_alloc()' is present],
	[idr_alloc_fn], [
...
#
AC_DEFUN([LIBCFS_SRC_HAVE_PERCPU_REFCOUNT], [
	LB2_LINUX_TEST_SRC([percpu_refcount], [
...
AC_DEFUN([LIBCFS_HAVE_PERCPU_REFCOUNT], [
	LB2_MSG_LINUX_TEST_RESULT([if percpu_refcount is available],
	[percpu_refcount], [
...
# v3.12
#
AC_DEFUN([LIBCFS_SRC_HAVE_SMP_STORE_LOAD], [
	LB2_LINUX_TEST_SRC([smp_load_acquire], [
...
AC_DEFUN([LIBCFS_HAVE_SMP_STORE_LOAD], [
	LB2_MSG_LINUX_TEST_RESULT([if 'smp_load_acquire()' is present],
	[smp_load_acquire], [
...
AC_DEFUN([LIBCFS_KTIME_GET_NS],[
	LB2_MSG_LINUX_TEST_RESULT([if function 'ktime_get_ns' exist],
	[ktime_get_ns], [], [
...
#
AC_DEFUN([LIBCFS_SRC_TIME64_TO_TM],[
	LB2_LINUX_TEST_SRC([fn_time64_to_tm_exists], [
		#include <linux/time.h>
	],[
...
AC_DEFUN([LIBCFS_TIME64_TO_TM],[
	LB2_MSG_LINUX_TEST_RESULT([if function 'time64_to_tm' exist],
...
AC_DEFUN([LIBCFS_UUID_T], [
	LB2_MSG_LINUX_TEST_RESULT([if 'uuid_t' exist],
	[uuid_t], [
...
AC_DEFUN([LIBCFS_PROG_LINUX_SRC], [
	# 3.9
	LIBCFS_SRC_HAVE_SOCKET_SK_SK_REUSEPORT
	LIBCFS_SRC_HAVE_IDR_ALLOC
	# 3.10
	LIBCFS_SRC_HAVE_PERCPU_REFCOUNT
	# 3.11
	LIBCFS_SRC_KTIME_GET_TS64
	# 3.12
	LIBCFS_SRC_HAVE_SMP_STORE_LOAD
