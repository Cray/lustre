 ...
 * If the LND for this message implements its own lnd_get_timeout() function
 * via its exposed API, we use this to calculate the LNet transaction timeout
 * (LTT) value, based on the message's NI LND timeout (LNDT) and global retry
 * count (LRC):
 * LTT = LNDT * (LRC + 1) + 1
 * If the LND did not implement the lnd_get_timeout() function or the LNDT was
 * set to zero, fall back to default global LTT implementation.
 */
