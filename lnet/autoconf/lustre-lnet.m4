...
case $with_o2ib in
	yes)    AS_IF([which ofed_info 2>/dev/null], [
			O2IBPATHS=$(ofed_info | egrep -w 'mlnx-ofa_kernel-devel|compat-rdma-devel|kernel-ib-devel|ofa_kernel-devel' | xargs rpm -ql | grep '\(/openib\|/ofa_kernel/default\)$'|head -1)
