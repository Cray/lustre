			 ...
			 int type)
{
	...
	CDEBUG(D_NET, "Add route type %d, ksnr_connected %x conns_per_peer %d\n",
	       type, route->ksnr_connected, *ksocknal_tunables.ksnd_conns_per_peer);
}
				  ...
				  unsigned char operstate)
{
	...
	list_for_each_entry_safe(net, cnxt, &ksocknal_data.ksnd_nets,
				 ksnn_list) {
		for (i = 0; i < net->ksnn_ninterfaces; i++) {
			if (strcmp(net->ksnn_interfaces[i].ksni_name,
			    dev->name) != 0)
				continue;
			ni = net->ksnn_ni;
			if (link_down)
