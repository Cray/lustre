...
unsigned int immediate_rx_buf_count = 4;
...
{
	if (!strncmp(tcstr, "best_effort", strlen("best_effort")))
		return KFI_TC_BEST_EFFORT;
	if (!strncmp(tcstr, "low_latency", strlen("low_latency")))
		return KFI_TC_LOW_LATENCY;
	if (!strncmp(tcstr, "dedicated_access", strlen("dedicated_access")))
		return KFI_TC_DEDICATED_ACCESS;
	if (!strncmp(tcstr, "bulk_data", strlen("bulk_data")))
		return KFI_TC_BULK_DATA;
	if (!strncmp(tcstr, "scavenger", strlen("scavenger")))
		return KFI_TC_SCAVENGER;
	if (!strncmp(tcstr, "network_ctrl", strlen("network_ctrl")))
...
int kfilnd_tunables_setup(struct lnet_ni *ni)
{
	...
	if (!ni->ni_lnd_tunables_set) {
		...
		if (strlen(traffic_class) < LNET_MAX_STR_LEN - 1)
