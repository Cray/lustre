...
int open_sysfs_file(const char *path, const char *attr, const int mode)
{
	...
	needed = snprintf(filename, sizeof(filename), "%s%s",
			  path, attr);
	if (needed >= LNET_MAX_STR_LEN)
		return -1;
				   ...
				   struct cYAML **err_rc)
{
	...
	switch (lnd) {
		case SOCKLND:
			rc = write_sysfs_file(socklnd_modparam_path,
					      "sock_timeout", val, 1,
					      strlen(val) + 1);
			break;
		case O2IBLND:
			rc = write_sysfs_file(o2iblnd_modparam_path,
					      "timeout", val, 1,
					      strlen(val) + 1);
			break;
		case KFILND:
			rc = write_sysfs_file(kfilnd_modparam_path,
					      "kfi_timeout", val, 1,
					      strlen(val) + 1);
			break;
		case GNILND:
